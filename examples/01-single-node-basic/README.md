...
1. Add the Node Wizard node to Node Client
   ...
   * Add the host
      * ```node-client -c register-server -S localhost --token [token]```
...
1. Add VM OS iso file(s) to Node Wizard
    * Find ISO files
        * Supported OSes
            * ```node-client -c iso-list -S localhost```
    * Add them to Node Wizard, e.g.
        * Ubuntu24.04:
            * ```node-client -S localhost -c iso-download -url https://old-releases.ubuntu.com/releases/24.04/ubuntu-24.04.1-live-server-amd64.iso```
        * Ubuntu22.04:
            * ```node-client -S localhost -c iso-download -url https://old-releases.ubuntu.com/releases/22.04/ubuntu-22.04.2-live-server-amd64.iso```
    * Check ISO files:
        * ```node-client -c iso-list -S localhost```
2. Create a VM
   * Write a json configuration file for a VM.
      * Template & example
         * ```node-client -desc vm-create```
   * Create an Ubuntu24.04 VM
      * Resource: 4 cores, 8G memory, 64G storage (Network: DHCP)
      * Login credentials: admin/changeme
      * Command
         * ```node-client -S localhost -c vm-create -v vm1 -f ubuntu24-basic.json```
   * Check VM creation status
      * ```node-client -S localhost -c list```
      * While the VM is being created, the vm state will be in running
      * When the VM is created, the vm state will be in shutdown
      * To access its console, use the following command.
         * ```virsh console vm1```
      * To get VNC access, use the following command for extracting port number and VNC password.
         * ```node-client -S localhost -c vncdisplay -v vm1```
         * Then, use your favoriate VNC client to connect.
3. Start using the VM
   * Start the VM
      * ```node-client -S localhost -c vm-start -v vm1```
   * Extract IP addresses from the VM
      * ```node-client -S localhost -c vm-domain-info -key ipaddr -v vm1```
   * Access the VM using ssh using the ip address obtained from the previous command (admin / changeme) 
